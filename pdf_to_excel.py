...
def extract_revenue_value(text: str, label: str) -> Optional[int]:
    ...
    if label_code_match:
        ...
        # Extract key words from the label for verification
        # E.g., "c Fundraising events" -> ["fundraising", "events"]
        label_keywords = [w.lower() for w in label.split()[1:] if len(w) > 3]
        ...
        for raw_line in text.splitlines():
            ...
            # Must contain the label keywords to ensure we're on the right line
            # This prevents matching "1c" on wrong lines
            if label_keywords and not any(keyword in line_lower for keyword in label_keywords):
                continue

            # Look for the code followed by a number with comma (indicates a value, not a reference)
            # Pattern should be: full_code followed by number at the END of the relevant text
            pattern = rf'\b{full_code}\b\s+(\d{{1,3}}(?:,\d{{3}})+|\d{{4,}})(?:\s|$)'
            match = re.search(pattern, line)
            if match:
                num_str = match.group(1).replace(",", "")
                try:
